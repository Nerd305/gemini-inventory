...
/**
 * Inventory core shared by the web app (src/lib/inventory.ts binds `db`) and the
 * Expo app (mobile/). Everything that touches Firestore takes the `db` handle as
 * its first argument so the same write path runs on both platforms.
 */
...
export async function getLocation(db: Firestore, locationId: string): Promise<FridgeLocation | null> {
...
export async function resolveLocationByCode(db: Firestore, parsed: ParsedCode): Promise<FridgeLocation | null> {
  if (parsed.value) {
    const byId = await getLocation(db, parsed.value).catch(() => null);
    if (byId) return byId;
  }
...
export async function getBasket(db: Firestore, basketId: string): Promise<BasketRecord | null> {
...
export async function resolveBasketByCode(db: Firestore, parsed: ParsedCode): Promise<BasketRecord | null> {
  if (parsed.value) {
    const byId = await getBasket(db, parsed.value).catch(() => null);
    if (byId) return byId;
  }
 ...
 */
export async function commitBasketCount(db: Firestore, input: CommitCountInput): Promise<CommitCountResult> {
  ...
  if (result.delta !== 0 && result.productId) {
    await adjustProductStock(db, result.productId, result.delta).catch((err) =>
...
export async function adjustProductStock(db: Firestore, productId: string, delta: number): Promise<void> {
 ...
 */
export async function createBasket(db: Firestore, input: CreateBasketInput): Promise<BasketRecord> {
  ...
  if (total > 0) {
    await adjustProductStock(db, input.productId, total).catch((err) =>
